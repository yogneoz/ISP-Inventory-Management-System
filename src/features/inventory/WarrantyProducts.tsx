...
export const WarrantyProducts: React.FC<WarrantyProductsProps> = ({
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div className="min-w-0">
          <h2 className="text-lg font-serif font-bold text-slate-900 dark:text-slate-100 tracking-tight flex items-center gap-2 break-words leading-tight">
            <ShieldCheck className={`h-5 w-5 ${isDarkMode ? 'text-emerald-400' : 'text-emerald-600'} shrink-0`} />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
        ...
        <div className={`rounded-2xl p-4 border shadow-xs transition-colors ${
          ...
          <div className="text-xs font-bold text-rose-800 dark:text-rose-400 break-words flex items-center gap-1.5">
            <ShieldAlert className={`h-4 w-4 ${isDarkMode ? 'text-rose-400' : 'text-rose-600'}`} />
      ...
      <div className={`p-3 rounded-2xl border shadow-xs flex flex-col md:flex-row md:items-center justify-start gap-3 ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`}>
 <div className="relative w-full md:w-80 lg:w-96 shrink-0">
          <Search className={`absolute left-3.5 top-1/2 -translate-y-1/2 h-4 w-4 ${isDarkMode ? 'text-emerald-400' : 'text-emerald-600'}`} />
          <input
            type="text"
            value={searchQuery}
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              className={`rounded-xl border px-3 py-2 text-xs font-semibold focus:outline-none focus:ring-2 focus:ring-emerald-500 ${isDarkMode ? 'border-slate-700 bg-slate-800 text-slate-200' : 'border-slate-200 bg-white text-slate-700'}`}
            >
              <option value="ALL" className={isDarkMode ? 'bg-slate-800 text-slate-100' : 'bg-white text-slate-900'}>All Statuses</option>
              <option value="VALID" className={isDarkMode ? 'bg-slate-800 text-emerald-400 font-semibold' : 'bg-white text-slate-900'}>VALID (Active)</option>
              <option value="EXPIRING_SOON" className={isDarkMode ? 'bg-slate-800 text-amber-400 font-semibold' : 'bg-white text-slate-900'}>EXPIRING SOON (&lt;30 Days)</option>
              <option value="EXPIRED" className={isDarkMode ? 'bg-slate-800 text-rose-400 font-semibold' : 'bg-white text-slate-900'}>EXPIRED</option>
            </select>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              className={`rounded-xl border px-3 py-2 text-xs font-semibold focus:outline-none focus:ring-2 focus:ring-emerald-500 ${isDarkMode ? 'border-slate-700 bg-slate-800 text-slate-200' : 'border-slate-200 bg-white text-slate-700'}`}
            >
              <option value="ALL" className={isDarkMode ? 'bg-slate-800 text-slate-100' : 'bg-white text-slate-900'}>All Types</option>
              <option value="CPE" className={isDarkMode ? 'bg-slate-800 text-blue-400 font-semibold' : 'bg-white text-slate-900'}>Customer CPE Devices</option>
              <option value="FIXED_ASSET" className={isDarkMode ? 'bg-slate-800 text-purple-400 font-semibold' : 'bg-white text-slate-900'}>Fixed Assets</option>
            </select>
          </div>
        </div>
      </div>
                  ...
                  return (
                    <tr key={`${item.type}-${item.id}`} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/50 transition-colors">
                      ...
                      <td className="p-2.5 text-center">
                        ...
                        >
                          {w.status === 'VALID' && <ShieldCheck className={`h-3.5 w-3.5 ${isDarkMode ? 'text-emerald-400' : 'text-emerald-600'} shrink-0`} />}
                          {w.status === 'EXPIRING_SOON' && <Clock className={`h-3.5 w-3.5 ${isDarkMode ? 'text-amber-400' : 'text-amber-600'} shrink-0`} />}
                          {w.status === 'EXPIRED' && <ShieldAlert className={`h-3.5 w-3.5 ${isDarkMode ? 'text-rose-400' : 'text-rose-600'} shrink-0`} />}
                          <span>{w.label}</span>
                        </div>
                      </td>
                    </tr>
